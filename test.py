...
def main():
    ...
    with torch.no_grad():
        ...
        for _ in tqdm(range(opts.number_eval)):
            ground_truth, mask, edge, gray_image, temp_image_file_name = next(image_data_loader)
            if isinstance(temp_image_file_name, tuple):
                image_file_name = temp_image_file_name[0]
            else:
                image_file_name = temp_image_file_name
            ...
            save_path = os.path.join(opts.result_root, image_file_name)
